...
// project documents edit modal show
...
function editPhaseDocument(id, user) {
    $.ajax({
        url: base_url + user + "/phase_document_details",
        type: 'POST',
        data: {'id': id},
        dataType: 'json',
        success: function (res) {
            $('#phase_document_id').val(res.id);
            $('#exist_file_name').val(res.enc_name);
            $('#ephase_document_title').val(res.title);
            $('#ephase_documents_remarks').val(res.remarks);
            mfp_modal('#editPhaseModal');
        }
    });
}
// project employee edit modal show
function editEmployee(id, user) {
    $.ajax({
        url: base_url + user + "/employee_details",
        type: 'POST',
        data: {'id': id},
        dataType: 'json',
        success: function (res) {
            $('#e_staffs_projects_id').val(res.id);
            $('#e_staffs_projects_employee_id').val(res.staff_id).trigger('change.select2');;
            $('#e_staffs_projects_remarks').val(res.remarks);
            mfp_modal('#editEmpModal');
        }
    });
}
...
function getPhaseDetails(id) {
    $.ajax({
        url: base_url + 'ajax/phase_details',
        type: 'POST',
        data: {'id': id},
        dataType: "json",
        success: function (data) {
            ...
            $('#ephase_name').val(data.name);
            $('#ephase_no').val(data.phase_no);
            $('#eproject_id').val(data.project_id).trigger('change');
            mfp_modal('#modal');
        }
    });
}
